...
{
    public static IAsyncEnumerable<T> EmptyAsync<T>() => EmptyAsyncEnumerable<T>.Instance;
...
#pragma warning restore IDE1006 // Naming rule violation: Missing suffix: 'Async'
    ...
    public static ValueTask<bool> AnyAsync<TSource>(this IAsyncEnumerable<TSource> source, Func<TSource, bool> predicate, CancellationToken cancellationToken = default)
    {
        ...
        return CoreAsync(source, predicate, cancellationToken);

        static async ValueTask<bool> CoreAsync(IAsyncEnumerable<TSource> source, Func<TSource, bool> predicate, CancellationToken cancellationToken)
        {
            await foreach (var item in source.WithCancellation(cancellationToken).ConfigureAwait(false))
            {
                if (predicate(item))
