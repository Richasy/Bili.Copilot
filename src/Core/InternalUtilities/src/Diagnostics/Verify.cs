...
#else
    private static Regex AsciiLettersDigitsUnderscoresRegex() => _asciiLettersDigitsUnderscoresRegex;
    private static readonly Regex _asciiLettersDigitsUnderscoresRegex = new("^[0-9A-Za-z_]*$", RegexOptions.Compiled);

    private static Regex FilenameRegex() => _filenameRegex;
