...
/// 流选择器.
/// </summary>
public unsafe class StreamSuggesterPlugin : PluginBase, ISuggestPlaylistItemPlugin, ISuggestAudioStreamPlugin, ISuggestVideoStreamPlugin, ISuggestSubtitleStreamPlugin, ISuggestSubtitlePlugin, ISuggestBestExternalSubtitlePlugin
{
    /// <inheritdoc/>
    public new int Priority { get; set; } = 3000;

    /// <inheritdoc/>
    public SubtitlesStream SuggestSubtitle(ObservableCollection<SubtitlesStream> streams, List<Language> langs)
    {
            ...
            foreach (var lang in Config.Audio.Languages)
            {
                foreach (var stream in streams)
                {
                    if (stream.Language == lang)
                    {
                        ...
                        for (var i = 0; i < stream.Demuxer.Programs.Count; i++)
                        {
                            bool aExists = false, vExists = false;
                            foreach (var pstream in stream.Demuxer.Programs[i].Streams)
                            {
                                if (pstream.StreamIndex == stream.StreamIndex)
                                {
                                    aExists = true;
                                }
                                else if (pstream.StreamIndex == stream.Demuxer.VideoStream?.StreamIndex)
                                {
                                    vExists = true;
                                }
                            }
    ...
    public VideoStream SuggestVideo(ObservableCollection<VideoStream> streams)
    {
        ...
        var iresults =
            ...
            where vstream.Type == MediaType.Video && vstream.Height <= Config.Video.MaxVerticalResolution
