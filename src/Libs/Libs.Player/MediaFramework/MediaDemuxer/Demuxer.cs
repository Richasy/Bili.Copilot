...
public unsafe class Demuxer : RunThreadBase
{
    ...
    public string Open(string url, Stream stream)
    {
        ...
        try
        {
            ...
            else if (url.StartsWith("fmt://") || url.StartsWith("device://"))
            {
                ...
                url = urlFromUrl;
                fmtStr = fmtStr.Replace("/", string.Empty);
            ...
            if (ret == AVERROR_EXIT || Status != ThreadStatus.Opening || Interrupter.ForceInterrupt == 1)
            ...
            else if (Type == MediaType.Subtitle && SubtitlesStreams.Count == 0)
                return error = $"No subtitles stream found";

            packet = av_packet_alloc();
            Status = ThreadStatus.Stopped;
            allowReadInterrupts = Config.AllowReadInterrupts && !Config.ExcludeInterruptFormats.Contains(Name);
    ...
    int IOOpen(AVFormatContext* s, AVIOContext** pb, string url, int flags, AVDictionary** avFmtOpts)
    {
        ...
        if (avoptCopy != null)
        {
            while ((t = av_dict_get(avoptCopy, string.Empty, t, AV_DICT_IGNORE_SUFFIX)) != null)
        ...
        //{
        //    while ((t = av_dict_get(*avFmtOpts, string.Empty, t, AV_DICT_IGNORE_SUFFIX)) != null)
    ...
    private void OpenFormat(string url, AVInputFormat* inFmt, Dictionary<string, string> opt, out int ret)
    {
        ...
        if (avopt != null && ret >= 0)
        {
            ...
            while ((t = av_dict_get(avopt, string.Empty, t, AV_DICT_IGNORE_SUFFIX)) != null)
    ...
    private bool FillInfo()
    {
        ...
        while (true)
        {
            b = av_dict_get(fmtCtx->metadata, string.Empty, b, AV_DICT_IGNORE_SUFFIX);
            if (b == null)
                break;
        ...
        for (int i = 0; i < fmtCtx->nb_chapters; i++)
        {
            ...
            while (true)
            {
                b = av_dict_get(chp->metadata, string.Empty, b, AV_DICT_IGNORE_SUFFIX);
                if (b == null)
                    break;
    ...
    protected override void RunInternal()
    {
        ...
        do
        {
            ...
            if (BufferedDuration > Config.BufferDuration || (Config.BufferPackets != 0 && CurPackets.Count > Config.BufferPackets))
            {
                ...
                while (!PauseOnQueueFull && (BufferedDuration > Config.BufferDuration || (Config.BufferPackets != 0 && CurPackets.Count > Config.BufferPackets)) && Status == ThreadStatus.QueueFull)
                    Thread.Sleep(20);

                lock (LockStatus)
                {
                    if (PauseOnQueueFull)
                    { PauseOnQueueFull = false; Status = ThreadStatus.Pausing; }
    ...
    private void RunInternalReverse()
    {
        ...
        do
        {
            ...
            if (VideoPacketsReverse.Count > maxQueueSize)
            {
                ...
                while (!PauseOnQueueFull && VideoPacketsReverse.Count > maxQueueSize && Status == ThreadStatus.QueueFull)
                { Thread.Sleep(20); }
