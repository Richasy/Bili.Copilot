...
{
    public ConcurrentQueue<VideoFrame> Frames { get; protected set; } = new ConcurrentQueue<VideoFrame>();
    ...
    internal static bool CheckCodecSupport(AVCodec* codec)
    {
        for (int i = 0; ; i++)
        {
            ...
            if (config->device_type == HW_DEVICE && config->pix_fmt == PIX_FMT_HWACCEL)
                return true;
        }
    ...
    internal int InitVA()
    {
        ...
        if (Renderer.Device == null || hw_device_ctx != null)
            return -1;
    ...
    private int AllocateHWFrames()
    {
        ...
        if (ret == 0)
        {
            lock (Renderer.lockDevice)
            {
                ...
                ZeroCopy = Config.Decoder.ZeroCopy == ZeroCopyState.Enabled || (Config.Decoder.ZeroCopy == ZeroCopyState.Auto && codecCtx->width == textureFFmpeg.Description.Width && codecCtx->height == textureFFmpeg.Description.Height);
                filledFromCodec = false;
            }
        }
    ...
    internal void RecalculateZeroCopy()
    {
        ...
        {
            bool save = ZeroCopy;
            ZeroCopy = VideoAccelerated && (Config.Decoder.ZeroCopy == ZeroCopyState.Enabled || (Config.Decoder.ZeroCopy == ZeroCopyState.Auto && codecCtx->width == textureFFmpeg.Description.Width && codecCtx->height == textureFFmpeg.Description.Height));
            if (save != ZeroCopy)
            {
    ...
    protected override void RunInternal()
    {
        ...
        do
        {
            ...
            if (Frames.Count >= Config.Decoder.MaxVideoFrames)
            {
                ...
                while (Frames.Count >= Config.Decoder.MaxVideoFrames && Status == ThreadStatus.QueueFull)
            ...
            if (demuxer.VideoPackets.Count == 0)
            {
                ...
                while (demuxer.VideoPackets.Count == 0 && Status == ThreadStatus.QueueEmpty)
                {
            ...
            lock (_lockCodecCtx)
            {
                ...
                if (ret != 0 && ret != AVERROR(EAGAIN))
                {
                    ...
                    {
                        if (demuxer.VideoPackets.Count > 0)
                        { avcodec_flush_buffers(codecCtx); continue; } // TBR: Happens on HLS while switching video streams
        ...
        if (isRecording)
        { StopRecording(); RecCompleted(MediaType.Video); }
    ...
    private void RunInternalReverse()
    {
        ...
        do
        {
            ...
            if (demuxer.VideoPacketsReverse.IsEmpty && curReverseVideoStack.IsEmpty && curReverseVideoPackets.Count == 0)
            {
                ...
                while (demuxer.VideoPacketsReverse.IsEmpty && Status == ThreadStatus.QueueEmpty)
                {
                    if (demuxer.Status == ThreadStatus.Ended) // TODO
                    {
                        lock (LockStatus)
                            Status = ThreadStatus.Ended;
            ...
            while (curReverseVideoPackets.Count > 0 && Status == ThreadStatus.Running)
            {
                // Wait until Queue not Full or Stopped
                if (Frames.Count + curReverseVideoFrames.Count >= Config.Decoder.MaxVideoFrames)
                {
                    ...
                    while (Frames.Count + curReverseVideoFrames.Count >= Config.Decoder.MaxVideoFrames && Status == ThreadStatus.QueueFull)
                        Thread.Sleep(20);
                ...
                lock (_lockCodecCtx)
                {
                    ...
                    {
                        if (ret == AVERROR_EOF)
                        { Status = ThreadStatus.Ended; break; }
    ...
    public VideoFrame GetFrame(int index)
    {
        ...
        demuxer.Interrupter.Request(Requester.Seek);
        ...
        if (demuxer.Status == ThreadStatus.Ended)
            demuxer.Status = ThreadStatus.Stopped;
        if (ret < 0)
            return null; // handle seek error
        Flush();
        keyFrameRequired = false;
    ...
    public int GetFrameNext(bool checkExtraFrames)
    {
        ...
        while (true)
        {
            ...
            if (ret != 0 && demuxer.Status != ThreadStatus.Ended)
                return ret;
            ...
            if (ret != 0)
            {
                if (allowedErrors < 1 || demuxer.Status == ThreadStatus.Ended)
                    return ret;
                allowedErrors--;
                continue;
            }
    ...
    private void DisposeFramesReverse()
    {
        ...
        {
            if (curReverseVideoPackets[i] == IntPtr.Zero)
                continue;
