...
namespace Danmaku.Core
{
    public static class BilibiliDanmakuXmlParser
    {
        ...
        public static List<DanmakuItem> GetSubtitleList(string jsonArrayStr)
        {
            ...
            {
                using var document = JsonDocument.Parse(jsonArrayStr);
                var root = document.RootElement;
                if(root.TryGetProperty("body", out var bodyArray))
                {
                    foreach (var jToken in bodyArray.EnumerateArray())
                    {
                        try
                        {
                            double fromMs = jToken.GetProperty("from").GetDouble() * 1000;
                            double toMs = jToken.GetProperty("to").GetDouble() * 1000;
                            if (toMs > fromMs)
                            {
                                string content = jToken.GetProperty("content").GetString();
